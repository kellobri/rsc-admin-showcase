---
title: "TensorFlow API Content Audit"
output: 
  html_document:
    theme: lumen
rmd_output_metadata:
  rsc_output_files:
    - "tensorflow-audit.csv"
---
...
## List all unsupported TensorFlow content

**Starting in Posit Connect version 2023.01.0, the TensorFlow Saved Model `app_mode` is unsupported**

After performing this upgrade:

- Configuration files that include `Server.TensorFlowEnabled`, will cause an error during Connect startup.
- Existing legacy TensorFlow Saved Model content will no longer run. An HTTP 502 error will be returned for all requests to these APIs.

Before you begin an upgrade to Posit Connect 2023.01.0, we recommend taking the following actions:

- Set a [System Message](https://docs.posit.co/connect/admin/server-management/#system-messages) on the server to warn Publishers about your upgrade timeline and the breaking change.
- Use this report to identify all unsupported TensorFlow content.
- Contact any users who have content that will break due to this upgrade. 
- Let Publishers know that moving forward TensorFlow models can be deployed as a [Plumber API](https://tensorflow.rstudio.com/guides/deploy/plumber.html), [Shiny application](https://tensorflow.rstudio.com/guides/deploy/shiny), Python API framework, or other supported content type. 
...
-   [Administrator API Key](https://docs.posit.co/connect/user/api-keys/#api-keys-creating){target="_blank"}
...
df_view <- df_full %>%
  ...
  filter(app_mode == "tensorflow-saved-model")
...
df_owners_view <- df_view %>%
  ...
  select(name, title, dashboard_url, app_mode, access_type, last_deployed_time, owner_guid, username)
```
...
write.csv(df_owners_view, "tensorflow-audit.csv", row.names=FALSE)
```

[tensorflow-audit.csv](tensorflow-audit.csv)
